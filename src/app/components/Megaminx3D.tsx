...
function MegaminxGroup({ cubeState }: { cubeState: CubeState }) {
  ...
  const meshes = useMemo(() => {
    if (!cubeState.megaminx || cubeState.megaminx.length < 132 || stickerData.length < 132) return null;
    return stickerData.map((st, i) => {
      const color = cubeState.megaminx[i] || '#000000';
      return (
        <group key={i}>
          <mesh geometry={st.baseGeom}>
            <meshStandardMaterial color="#050505" roughness={0.8} metalness={0.1} side={THREE.DoubleSide} />
          </mesh>
  ...
  return <group ref={groupRef} scale={[1.2, 1.2, 1.2]}>{meshes}</group>;
}
  ...
  return (
    <div className="w-full h-full bg-gray-900 rounded-lg overflow-hidden relative cursor-grab active:cursor-grabbing">
      <Canvas camera={{ position: [0, 2, 6], fov: 45 }} gl={{ antialias: true }}>
        ...
        <TrackballControls noPan={true} minDistance={3} maxDistance={10} dynamicDampingFactor={0.1} rotateSpeed={4.0} />
      </Canvas>
    </div>
  );
}